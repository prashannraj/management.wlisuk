  ...
  <div class="col">
    ...
    <div class="card" id="documentCard">
      <div class="card-header">
        <h3 class="text-primary font-weight-600">Templates
          <a href="{{ route('template.create') }}" class="btn btn-sm btn-primary float-right" id="addBtnDocument">
            <span class="d-sm-block d-md-none"> <i class="fas fa-plus"></i></span>
            <span class="d-none d-md-block d-lg-block"> <i class="fas fa-plus"></i> Add new template </span>
          </a>
        </h3>
      </div>
      <div class="card-body">
          ...
          <table class="table table-border table-striped" id="invoice_table">
            ...
            <tbody class="">
              ...
              <tr>
                ...
                <td> {{$template->short_text}} </td>
                <td>
                  @php
                  $editUrl = route('template.edit',$template->id);
                  $viewUrl = route('template.show', $template->id);
                  ...
                  @endphp
                  {!!$btn!!}
                <a href="#delete_template" data-toggle="modal" data-id="{{$template->id}}" class="btn btn-warning btn-sm">
      ...
      <div class="card-footer">
        <a href="{{ route('template.create') }}" class="btn btn-sm btn-primary" id="addBtnDocument">
          <span class="d-sm-block d-md-none"> <i class="fas fa-plus"></i> D</span>
          <span class="d-none d-md-block d-lg-block"> <i class="fas fa-plus"></i> Create new template </span>
        </a>
      </div>
    </div>
  </div>
</div>
<div id="delete_template" class='modal fade'>
  <div class="modal-dialog modal-confirm">
    <div class="modal-content">
      ...
      <form action="{{ route('template.destroy','_')}}" method="POST">
        @csrf
        @method("DELETE")
        <div class="modal-body">
          <input id="data_id" name="id" type="hidden">

          <p>Do you really want to delete this template? This process cannot be undone.</p>
...
<script>
  $("#delete_template").on('show.bs.modal', function(event) {
