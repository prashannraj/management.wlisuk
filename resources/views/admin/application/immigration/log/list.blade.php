...
<div class="card" id="documentCard">
    <div class="card-header">
        <h4 class="text-primary font-weight-600">Application Processes for {{$data['application']->student_name}}
            ...
            <table class="table table-border table-striped" id="documentTable">
                ...
                <tbody class="document_details">
                    ...
                    <tr>
                        ...
                        <td>{{$logs->applicationStatus->title}} @if($logs->application_status_id == config('constant.file_closed_id')) - {{$logs->reason}} @endif </td>
                        ...
                        <td>
                            <a href="{{ route('client.application.view.immigration.process', $logs->id) }}" data-target="#view_application_process" data-id="{{$logs->id}}" data-application_status_id="{{$logs->application_status_id}}" data-note={{base64_encode($logs->note)}} data-document="{{$logs->document?$logs->file_url: '-1'}}" class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a>
        ...
        <div class='modal-content'>
            ...
            <div class='modal-body'>
                <div class='border shadow-sm p-2'>
                    <form action="{{route('client.list.application.immigration.store,logs',['id'=>$data['application']->id])}}" enctype='multipart/form-data' method="POST">
                        @csrf
...
<div class='modal' id="update_application_process">
    <div class='modal-dialog'>
        <div class='modal-content'>
            ...
            <div class='modal-body'>
                <div class='border shadow-sm p-2'>
                    <form action="{{route('client.application.update.immigration.process',['id'=>'_'])}}" enctype='multipart/form-data' method="POST">
                        @csrf
                        @method("PUT")
                        <input type="hidden" value="" id="data-id" name="application_process_id">
                        <div class="form-group">
...
<div class="modal fade" id="deleteApplicationProcess" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{route('client.application.delete.immigration.process','_')}}" method="post">
                {{method_field('put')}}
                {{csrf_field()}}
                <div class="modal-body">
                    <p class="text-center">
...
<div class="modal fade" id="sendEmail" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{route('client.application.email.immigration.process')}}" enctype="multipart/form-data" method="post">
                {{csrf_field()}}
                <div class="modal-body">
...
<script>
    ...
    $('#update_application_process').on('show.bs.modal', function(event) {
        ...
        if(application_status == {{config('constant.file_closed_id')}}){
            modal.find('.modal-body .reason').show();
        }
...
<script>
    $(".application_status").on('change',function(evt){
        var x=$(this).val();
