            ...
            <table class="table table-border table-striped" id="documentTable">
                ...
                <tbody class="document_details">
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('client.application.view.immigration-appeal.process', $logs->id) }}" data-target="#view_application_process" data-id="{{$logs->id}}" data-application_status_id="{{$logs->application_status_id}}" data-note={{base64_encode($logs->note)}} data-document="{{$logs->document?$logs->file_url: '-1'}}" class="btn btn-info btn-sm"><i class="fa fa-eye"></i></a>
                            ...
                            <a href="{{ route('client.edit.application.immigrationappeal', $logs->id) }}" data-date="{{$logs->date}}" data-reason="{{$logs->reason}}" data-toggle="modal" data-target="#update_application_process" data-id="{{$logs->id}}" data-application_status_id="{{$logs->application_status_id}}" data-note="{{base64_encode($logs->note)}}" data-document="{{$logs->document?$logs->file_url: '-1'}}" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></a>
        ...
        <div class='modal-content'>
            ...
            <div class='modal-body'>
                <div class='border shadow-sm p-2'>
                    <form action="{{route('client.list.application.immigration-appeal.storelogs',['id'=>$data['application']->id])}}" enctype='multipart/form-data' method="POST">
                        @csrf
...
<div class='modal' id="update_application_process">
    <div class='modal-dialog'>
        <div class='modal-content'>
            ...
            <div class='modal-body'>
                <div class='border shadow-sm p-2'>
                    <form action="{{route('client.application.update.immigration-appeal.process',['id'=>'_'])}}" enctype='multipart/form-data' method="POST">
                        @csrf
                        @method("PUT")
                        <input type="hidden" value="" id="data-id" name="application_process_id">
                        <div class="form-group">
...
<div class="modal fade" id="deleteApplicationProcess" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="{{route('client.application.delete.immigration-appeal.process','_')}}" method="post">
                {{method_field('put')}}
                {{csrf_field()}}
                <div class="modal-body">
                    <p class="text-center">
