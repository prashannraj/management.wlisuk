            ...
            <div class="header-body">
                <div class="row align-items-center py-4">
                    <div class="col-lg-6 col-7">
                        <h6 class="h2 text-white d-inline-block mb-0">Send client care for {{ $data['enquiry']->full_name }}
                        </h6>
                    </div>
...
@section('main-content')
    ...
    <div class="card" id="clientcare_form">
        <div class="card-header">
            <h4 class='text-primary'>Generate Client Care</h4>
        </div>

        <div class="card-body">
            <form action="{{ route('enquiry.clientcare', $data['enquiry']->id) }}" enctype="multipart/form-data"
                method="post">
                @csrf
                <div class="row">
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <!-- Display the full name with the added names (if any) -->

                            <p id="fullNameDisplay">

                                @if(($data['clientcare'] ['full_name_with_title'])!== null)
                                {{$data['clientcare'] ['full_name_with_title']}}
                                @else
                                {{ $data['enquiry']->full_name_with_title }}
                                @endif
                            </p>


                            <!-- The input field for editing the name -->
                            <input type="text" id="nameField" class="form-control" value="{{ $data['enquiry']->full_name_with_title }}">


                            <!-- Hidden input to store the added names -->
                            <input type="hidden" id="added_names_input" name="added_names_input" value="{{ json_encode($data['clientcare']->added_names_input) }}">

                            <!-- Button to add new name -->
                            <button type="button" id="addNameButton" class="btn btn-primary mt-2" data-bs-toggle="modal" data-bs-target="#addNameModel">Add Name</button>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control datepicker2"
                                value="{{ old('date', optional($data['clientcare'])->date) ?? date('d/m/Y') }}"
                                name='date'>
                        </div>
                    </div>
                        ...
                        <div class="form-group">
                            ...
                            <select name="advisor_id" class="form-control">
                                ...
                                @foreach ($data['advisors'] as $advisor)
                                    <option
                                        {{ old('advisor_id', optional($data['clientcare'])->advisor_id) == $advisor->id ? 'selected' : '' }}
                        ...
                        <div class="form-group">
                            ...
                            <select name="bank_id" class="form-control">
                                ...
                                @foreach ($data['banks'] as $bank)
                                    <option
                                        {{ old('bank_id', optional($data['clientcare'])->bank_id) == $bank->id ? 'selected' : '' }}
                        ...
                        <div class="form-group">
                            ...
                            <select name="agreed_fee_currency_id" class="form-control">
                                ...
                                @foreach ($data['currencies'] as $currency)
                                    <option
                                        {{ old('agreed_fee_currency_id', optional($data['clientcare'])->agreed_fee_currency_id) == $currency->id ? 'selected' : '' }}
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="text"
                                value="{{ old('agreed_fee', optional($data['clientcare'])->agreed_fee) }}"
                    ...
                    <div class="col-md-12"> <!-- Full width for new textarea -->
                        <div class="form-group">
                            ...
                            <textarea id="additional_notes" name="additional_notes" class='form-control wysiwyg' rows='3'
                                placeholder="Enter any additional notes here...">{{ old('additional_notes', optional($data['clientcare'])->additional_notes) }}</textarea>
                        </div>
                    </div>
                    <div>
                        <button class="template-load btn-small btn-primary mx-2" id="loadButton" data-enquiry-id="{{ $data['enquiry']->id }}">Load</button>
                        {{-- <button class="template-load btn-small btn-primary mx-2" onclick="hello()">Load</button> --}}
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <p>{!! $data['discussion_content'] ?? 'Default content here' !!}</p>
                            ...
                            <p style="margin-bottom: 0.14in; line-height: 120%;" align="center"><span style="font-family: 'Courier New', Courier, monospace;"><strong>@if($data['bank'])
                                <div>
                                    {!! $data['bank']->formattedDetails() !!}
                                </div>
                            @else
                                <p>No bank information available.</p>
                            @endif</strong></span> <span style="font-family: 'Courier New', Courier, monospace;"><strong></strong></span></p>
                ...
                <input type="hidden" name="added_names" id="added_names_input">
                <div>

                    <input type="submit" formtarget="_blank" name="action" value="Download"
                        class="btn btn-warning float-right">
                    <input type="submit" formtarget="_blank" name="action" value="Preview"
                ...
                <div class="card">
                    <div class="card-body">
                        ...
                        <div class="attachment-list">
                            <label for="">Additional Attachments (if any)</label>
                        ...
                        <br>
                        <br>
                        <input type="submit" name="action" value="Email" class="btn btn-primary">
...
@push('scripts')
   ...
   <script>
    ...
    fetch(`/enquiry/${enquiryId}/load-data`) // Adjust the URL as necessary
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                // Populate the fields with the retrieved data
                document.getElementById('nameField').value = data.data.full_name_with_title;
                document.getElementById('added_names_input').value = JSON.stringify(data.data.added_names);
                document.querySelector('textarea[name="full_address"]').value = data.data.full_address;
                document.querySelector('input[name="date"]').value = data.data.date;
                document.querySelector('select[name="agreed_fee_currency_id"]').value = data.data.agreed_fee_currency_id;
                document.querySelector('select[name="servicefee_id"]').value = data.data.servicefee_id;
                document.querySelector('input[name="agreed_fee"]').value = data.data.agreed_fee;
                document.querySelector('select[name="bank_id"]').value = data.data.bank_id;

                // Set the additional_notes in the textarea
                document.querySelector('textarea[name="additional_notes"]').value = data.data.additional_notes;

                // Set the discussion_content in the TinyMCE editor
                tinymce.get('discussion_details').setContent(data.data.discussion_content || '');
    ...
    <script>
        ...
        function hello() {
            // Get the value from the 'test' textarea
            const testValue = tinymce.get('additional_notes').getContent();


            // Get the content of the TinyMCE editor
            const editorContent = tinymce.get('discussion_details').getContent();

            // Parse the editor content as HTML
            const parser = new DOMParser();
            const doc = parser.parseFromString(editorContent, 'text/html');

            // Find the paragraph containing "Affordable Pricing"
            const targetParagraph = Array.from(doc.querySelectorAll('h1'))
                .find(p => p.textContent.includes("INSTRUCTIONS"));

            if (targetParagraph) {
                // Create a new element with the test value
                const newElement = document.createElement('p');
                newElement.innerHTML = testValue;

                // Insert the new element after the target paragraph
                targetParagraph.parentNode.insertBefore(newElement, targetParagraph.nextSibling);
            } else {
                console.warn("Paragraph containing 'Affordable Pricing' not found.");
                return;
            }

            // Serialize the updated content back into a string
            const updatedContent = doc.body.innerHTML;

            // Update the TinyMCE editor content
            tinymce.get('discussion_details').setContent(updatedContent);
    ...
    <script>
        ...
        $('.template-load').on('click', function(evt) {
            evt.preventDefault();
            ...
            ajaxSearchTemplate(id, loading, target);
        });
    ...
    <script>
        ...
        @if (optional($data['clientcare'])->servicefee_id != null)
            $('.servicefeeautocomplete').autoComplete('set', {
                value: "{{ $data['clientcare']->servicefee_id }}",
                text: "{{ $data['clientcare']->servicefee->name }} - {{ $data['clientcare']->servicefee->category }}"
            });
        @endif
...
document.addEventListener("DOMContentLoaded", function() {
    // Initialize names list with the current full name from Blade variable
    let namesList = ["{{ $data['enquiry']->full_name_with_title }}"];
    ...
    // Initialize the modal using Bootstrap's JS modal function
    const nameModal = new bootstrap.Modal(document.getElementById("addNameModel")); // Corrected here

    // Check if the added names are already set (from the hidden input field)
    let addedNames = JSON.parse(addedNamesInput.value);

    // If addedNames is empty, show enquiry's full name, otherwise show client care's full name
    if (addedNames.length === 0) {
        nameField.value = "{{ $data['enquiry']->full_name_with_title }}";
        fullNameDisplay.textContent = "{{ $data['enquiry']->full_name_with_title }}";
    } else {
        nameField.value = "{{ $data['clientcare']->full_name_with_title }}";
        fullNameDisplay.textContent = "{{ $data['clientcare']->full_name_with_title }}";
    }

    // When the Add Name button is clicked, show the modal
    addNameButton.addEventListener("click", function() {
        nameModal.show(); // This should work correctly now
    });

    // When the Save Name button inside the modal is clicked
    saveNameButton.addEventListener("click", function() {
        const newName = newNameInput.value.trim();

        if (newName) {
            namesList.push(newName); // Add the new name to the list
            updateNameDisplay();
            updateHiddenInput(); // Update hidden input after adding name
            updateTablesInWYSIWYG();
            newNameInput.value = '';
            nameModal.hide(); // Hide the modal after saving the name
        }
    });
    ...
    // Other functions remain unchanged...
        ...
        // Function to update the name display with proper formatting
        function updateNameDisplay() {
            let displayText = '';
            if (namesList.length === 1) {
                displayText = namesList[0];
            } else if (namesList.length === 2) {
                displayText = `${namesList[0]} & ${namesList[1]}`;
            } else {
                displayText = namesList.slice(0, -1).join(', ') + ' & ' + namesList[namesList.length - 1];
            }

            fullNameDisplay.textContent = displayText;
            nameField.value = displayText;
        }

        // Function to update hidden input with all names
        function updateHiddenInput() {
            addedNamesInput.value = JSON.stringify(namesList);
        }

        // Function to update tables in WYSIWYG editor
        function updateTablesInWYSIWYG() {
            ...
            // Create new table row HTML for the new name
            const newTableRow = `
    <tr>
        <td>
    </tr>
    <tr>
            ...
            // Find and update the first table (Client Care Letter)
            let firstTablePosition = content.indexOf("Client Care Letter agreeing");
            if (firstTablePosition !== -1) {
                let firstTableStart = content.indexOf("<table", firstTablePosition);
                let firstTableEnd = content.indexOf("</table>", firstTableStart);
                let firstTbodyStart = content.indexOf("<tbody>", firstTableStart);
                let firstTbodyEnd = content.indexOf("</tbody>", firstTbodyStart);

                if (firstTbodyStart !== -1 && firstTbodyEnd !== -1) {
                    content = content.slice(0, firstTbodyEnd) + newTableRow + content.slice(firstTbodyEnd);
                }
            }

            // Find and update the second table (Terms and Conditions)
            let secondTablePosition = content.indexOf("By signing below");
            if (secondTablePosition !== -1) {
                let secondTableStart = content.indexOf("<table", secondTablePosition);
                let secondTableEnd = content.indexOf("</table>", secondTableStart);
                let secondTbodyStart = content.indexOf("<tbody>", secondTableStart);
                let secondTbodyEnd = content.indexOf("</tbody>", secondTbodyStart);

                if (secondTbodyStart !== -1 && secondTbodyEnd !== -1) {
                    content = content.slice(0, secondTbodyEnd) + newTableRow + content.slice(secondTbodyEnd);
                }
            }

            // Add debug logs
            console.log("First table found at position:", firstTablePosition);
            console.log("Second table found at position:", secondTablePosition);
