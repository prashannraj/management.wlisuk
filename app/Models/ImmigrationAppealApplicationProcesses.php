...
class ImmigrationAppealApplicationProcesses extends Model
{
    protected $table = 'immigration_appeal_application_processes';
    ...
    public function application(){
        return $this->belongsTo(ImmigrationAppealApplication::class,'application_id','id');
    }
    ...
    public function getElapsedDaysAttribute()
    {
        ...
        $now = Carbon::now();
        $e = "";
        $p = \App\Models\ImmigrationAppealApplicationProcesses::where("application_status_id",">",$this->application_status_id)->where('application_id',$this->application_id)->orderBy("application_status_id","asc")->first();
        if($p){
        	$now = $p->created;
        	$e = $p->id;
