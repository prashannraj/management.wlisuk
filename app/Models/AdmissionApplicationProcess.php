...
class AdmissionApplicationProcess extends Model
{
    protected $table = 'admission_application_processes';
    ...
    public function getElapsedDaysAttribute()
    {
        ...
        $now = Carbon::now();
        $p = \App\Models\AdmissionApplicationProcess::where("application_status_id",">",$this->application_status_id)->where('application_id',$this->application_id)->orderBy("application_status_id","asc")->first();
        if($p){
        	$now = $p->created;
        	$e = $p->id;
