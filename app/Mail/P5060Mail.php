...
class P5060Mail extends Mailable
{
    ...
    {
        $data = $this->data;
        $type = strtoupper($data['row']->type);
        $view  = 'admin.emails.'.$data['row']->type;
        $subject = "Att: {$data['row']->employee->full_name} ({$data['row']->employee_id}) $type [{$data['row']->year}]";
        $attachments_string = '';
        $full_path  = $data['row']->document;
        $extension =explode('.',$full_path );
        $extension = end($extension);

        $filename = "$type " . $data['row']->year . '-' . $data['row']->employee->full_name . ".$extension";
        $initial =  $this
            ->subject($subject)->view($view, compact('data'));
