...
use App\Models\ImmigrationAppealApplication;
use App\Models\ImmigrationAppealApplicationProcesses;
...
use App\Models\ImmigrationAppealApplicationStatus;
use Illuminate\Support\Facades\Mail;
use Illuminate\Support\Facades\Storage;
use PDF;


class ImmigrationAppealController extends BaseController
{
    private $title;
    ...
    public function addImmigrationAppeal($basic_info_id)
    {
        ...
        $data = [];
        $data['panel_name'] = 'Add Immigration Appeal Application';
        $data['row'] = $basic_info;
        $data['country_code'] = $this->country_code;
        $data['advisors'] = Advisor::where('status', 'active')->get();
        return view('admin.application.immigration-appeal.add', compact('data'));
    }

    public function storeImmigrationAppeal(Request $request)
    {
        ...
        try {
            ...
            if (!$request->basic_info_id || !$basic_info) {
                return "We cannot proccess immigration appeal process";
            }
            ...
            $immigrationApplication = new ImmigrationAppealApplication();
            ...
            $immigrationApplication->application_type = $request->application_type;
            ...
            if ($immigrationApplication->save()) {
                $ImmigrationApplicationProcess = new ImmigrationAppealApplicationProcesses();
                ...
                Session::flash('success', 'Immigration Appeal Application has been created.');
    ...
    public function editImmigrationAppeal($applicationId)
    {
        $application = ImmigrationAppealApplication::find($applicationId);
        if (!$applicationId || !$application) {
            return "We cannot proccess immigration application appeal process";
        }

        $data = [];
        $data['panel_name'] = 'Edit Immigration Appeal Application #' . $application->basicInfo->full_name;
        $data['row'] = $application;
        $data['country_code'] = $this->country_code;
        $data['advisors'] = Advisor::where('status', 'active')->get();

        return view('admin.application.immigration-appeal.edit', compact('data'));
    }

    public function updateImmigrationAppeal(Request $request, $applicationId)
    {
        $validator = \Validator::make($request->all(), [
            'student_name' => "required",
            "basic_info_id"      => "required|exists:basic_infos,id",
        ...
        try {
            ...
            $immigrationApplication = ImmigrationAppealApplication::find($applicationId);
            if (!$immigrationApplication || !$immigrationApplication) {
                return "We cannot proccess immigration application process";
            }
            ...
            if ($immigrationApplication->save()) {
                ...
                $ImmigrationApplicationProcess->modified_by = $userId;
                $ImmigrationApplicationProcess->save();
                Session::flash('success', 'Immigration Appeal Application has been updated.');
                return redirect()->route("client.show", ['id'=>$request->basic_info_id,'click'=>'applications']);
            ...
            Session::flash('failed', 'Immigration Appeal could not be updateds. Due to ' . $e->getMessage() . ' error.');
            return redirect()->back();
        }
    }

    public function immigrationAppealLogs($applicationId)
    {
        $application = ImmigrationAppealApplication::find($applicationId);
        if (!$applicationId || !$application) {
            return "We cannot proccess immigration application process";
        ...
        $data['applicationLogsStatuses'] = \App\Models\ImmigrationAppealApplicationStatus::get();
        // dd($data['applicationLogs']);
        $data['country_code'] = $this->country_code;
        return view('admin.application.immigration-appeal.log.list', compact('data'));
    }

    public function storeImmigrationAppealLogs(Request $request, $applicationId)
    {
        ...
        $i = ImmigrationAppealApplicationProcesses::create($data);
    ...
    public function updateApplicationImmigrationProcess(Request $request, $id)
    {
        ...
        try {
            ...
            $applicationProcess = ImmigrationAppealApplicationProcesses::find($data['application_process_id']);
    ...
    {
        $applicationProcess = ImmigrationAppealApplicationProcesses::find($id);
        if (!$applicationProcess) {
            return "We cannot proccess immigration application process";
        }
    ...
    {
        $data  = $request->all();
        $applicationProcess = ImmigrationAppealApplicationProcesses::findOrFail($data['application_process_id']);
    ...
    public function emailApplicationImmigrationProcess(Request $request)
    {
        ...
        $data['application_process'] = ImmigrationAppealApplicationProcesses::with('application.client')->findOrFail($data['application_process_id']);
    ...
    public function destroy(Request $request){
        ...
        $im  = ImmigrationAppealApplication::findOrFail($id);
